...
class TxnService {
  ...
  Future<TxnData> processTxn(String service, String msisdn, double amount,
      {String ref = "", String meterNumber = ""}) async {
    ...
    try {
      final res = await http.post('$API_BASE_URL/api/v1/transaction',
  ...
  Future<TxnData> getTxn(int txnId) async {
    ...
    try {
      final res = await http.get(
        '$API_BASE_URL/api/v1/transaction?id=$txnId',
        headers: {'Content-Type': 'application/json', 'token': token},
      );
      ...
      if (res.statusCode == 200) {
        ...
        if (resData['is_error']) {
          ...
          _txnData = TxnData.fromJson(resData['transaction']);
          return Future.value(_txnData);
        }
      } else {
        _txnData = TxnData(isError: true);
